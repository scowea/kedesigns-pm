...
DataGrid.extend = function(obj) {
  for (var i in obj) {
    if (this[i]) {
      DataGrid.extend.apply(this[i], [obj[i]]);
    }
    else {
      this[i] = obj[i];
    }
 ...
 */
DataGrid.redirectFormButton = function (uri, button, handler) {
  // Trap the button
  button.onmouseover = button.onfocus = function() {
    button.onclick = function() {
      // Create target iframe
      DataGrid.createIframe();
      ...
      window.iframeHandler = function () {
        ...
        response = DataGrid.parseJson(response);
        // Check response code
 ...
 */
DataGrid.absolutePosition = function (el) {
  var sLeft = 0, sTop = 0;
  ...
  if (el.offsetParent) {
    var tmp = DataGrid.absolutePosition(el.offsetParent);
    r.x += tmp.x;
    r.y += tmp.y;
  }
  return r;
 ...
 */
DataGrid.dimensions = function (el) {
 ...
 */
DataGrid.parseJson = function (data) {
 ...
 */
DataGrid.createIframe = function () {
 ...
 */
DataGrid.deleteIframe = function () {
 ...
 */
DataGrid.freezeHeight = function () {
  DataGrid.unfreezeHeight();
  var div = document.createElement('div');
  $(div).css({
    position: 'absolute',
    top: '0px',
 ...
 */
DataGrid.unfreezeHeight = function () {
  $('#freeze-height').remove();
};

/**
 * Wrapper to address the mod_rewrite url encoding bug
 * (equivalent of DataGrid_urlencode() in PHP).
 */
DataGrid.encodeURIComponent = function (item, uri) {
